...
namespace ContactModels
{
  ...
  public:
    ...
    {
      registry.registerProperty("surfaceEnergySint", &MODEL_PARAMS::createSurfaceEnergySint,"model sintering");
      registry.registerProperty("diffusionParameter", &MODEL_PARAMS::createDiffusionParameter,"model sintering");

       registry.connect("surfaceEnergySint", surfaceEnergy,"model sintering");
      registry.connect("diffusionParameter", difusionParam,"model sintering");
    ...
    inline void surfacesIntersect(SurfacesIntersectData & sidata, ForceData & i_forces, ForceData & j_forces)
    {
      ...
      // if(sidata.deltan > reff*0.535898384862)
      //   error->one(FLERR, "sidata.deltan > reff*0.535898384862!");
      ...
      const double Fn_sintering = -1.125 *2*M_PI*reff*surfaceEnergy;
      ...
      if (!disable_when_bonded_ || sidata.contact_history[bond_history_offset_] < 0.5)
      {
        ...
        if(heating)
        {
            ...
            if(heating_track && sidata.is_wall)
                cmb->tally_pw(E_therm ,sidata.i,sidata.jtype,0);
            if(heating_track && !sidata.is_wall)
